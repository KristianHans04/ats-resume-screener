...
function DeltaBadge({ delta }) {
  ...
  const styles = direction === 'up' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' :
                 direction === 'down' ? 'text-red-700 bg-red-50 border-red-200' :
...
export default function MetricWidget({
  ...
  const iconColors = {
    ...
    sapphire: 'bg-gray-50 border-gray-200 text-gray-500',
    ...
    <Card variant={cardVariant} padding="md">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <p className="font-mono text-xs tracking-widest uppercase text-gray-500 leading-none">{label}</p>
          {icon && (
        ...
        <div className="flex items-end gap-2">
          <span className={`font-mono text-3xl font-medium leading-none tabular-nums tracking-tight ${variant === 'gold' ? 'text-accent' : 'text-neutral-dark'}`}>
            {value}
          </span>
          {unit && <span className="font-mono text-sm text-gray-400 pb-0.5 leading-none">{unit}</span>}
        </div>
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-0">
              {delta !== null && (
                <div className="flex items-center gap-2">
                  ...
                  {deltaLabel && <span className="text-xs text-gray-400 truncate">{deltaLabel}</span>}
                </div>
              )}
              {description && <p className="text-[11px] text-gray-400 leading-snug pr-2">{description}</p>}
            </div>
