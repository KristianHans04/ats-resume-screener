...
// POST /api/jobs/applications/:id/reject – reject a candidate (recruiter only)
export async function onRequestPost(context) {
  ...
  if (user.role !== 'RECRUITER') {
    return errorResponse('Only recruiters can reject candidates', 403);
  }
  ...
  return jsonResponse({ status: 'Candidate rejected.' });
}
